...
type processOptions struct {
	...
	SenderDisplayName       string   // Current sender display name for dynamic contextUserMessage
	...
	if !opts.NoHistory {
		...
		if isOverContextBudget(agent.ContextWindow, messages, toolDefs, agent.MaxTokens) {
			...
			messages = agent.ContextBuilder.BuildMessages(
				...
				opts.Media, opts.Channel, opts.ChatID, opts.SenderID, opts.SenderDisplayName,
			)
			messages = resolveMediaRefs(messages, al.mediaStore, maxMediaSize)
		}
	}
